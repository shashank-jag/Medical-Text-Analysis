...
for line in entries:
	...
	#...........unhandled .............................
	if pres_entry[-1] == '0':
		pres_entry[0] = pres_entry[0].lower()
		if pres_entry[0] == "injection" or pres_entry[0] == "po" or pres_entry[0] == "PO" or pres_entry[0] == "p.o." or pres_entry[0] == "eye" or pres_entry[0] == "nasal" or pres_entry[0] == "inh" or pres_entry[0] == "sc" :
			pres_entry[-1] = "mode"
		else :
			continue
	word = pres_entry[0]
...
for i in range(len(finalOutput)):
	...
	if fl:
		continue
	if x[3].strip() == "the" or x[3].strip() == "her" or x[3].strip() == "his" or x[3].strip() == "her." or x[3].strip() == "his." or len(x[3].strip())<=3 :
		continue
	x = [str(i) for i in x]
	s = x[0]+'|'+x[1]+'-'+x[2]+'|'+x[3]+'|'+x[4]
	print(s)

	'''Relaxed:    Precision:0.8976982097186701  Recall :0.41245593419506466 Fmeasure :0.5652173913043479
Strict :    Precision:0.8849104859335039  Recall :0.40658049353701525 Fmeasure :0.5571658615136875'''
